(() => {
  ...
  function updateContact(){setText('#contactSectionTitle','Visit the Kundiawa Base or Book a Mobile Service');setText('#contactSectionSubtitle','Stationery and printing are based in Kundiawa. Mobile barber and business assistance services are available by appointment.');const phone=document.querySelector('#contactPhone');if(phone){phone.href='tel:+67579243217';phone.innerHTML='<i class="fas fa-phone-alt"></i><span>7924 3217</span>';}let email=document.querySelector('#contactEmail');const contactList=document.querySelector('.contact-details-list');if(!email&&contactList){email=document.createElement('a');email.id='contactEmail';email.className='contact-link';contactList.appendChild(email);}if(email){email.href=`mailto:${EMAIL}`;email.innerHTML=`<i class="fas fa-envelope"></i><span>${EMAIL}</span>`;}const whatsapp=document.querySelector('#contactWhatsApp');if(whatsapp){whatsapp.style.display='flex';whatsapp.href=wa(generalMessage);whatsapp.innerHTML='<i class="fab fa-whatsapp"></i><span>WhatsApp Us</span>';}setText('.contact-hours','Monday–Thursday 7AM–5PM | Friday 7AM–12PM | Sunday 1PM–5PM');const address=document.querySelector('.contact-info .contact-card:not(.contact-card-combined) .contact-details');if(address)address.innerHTML='<h3>Stationery & Printing Base</h3><div class="bnb-address"><p class="bnb-address-name">BNB Stationery, Printing & Consultancy Services</p><p>Warasimbu Travellers Inn<br>Kundiawa, Simbu Province<br>Papua New Guinea</p><p><strong>Mobile by appointment:</strong> Barber and business assistance services.</p></div><a class="bnb-map-placeholder" href="https://www.google.com/maps/search/?api=1&query=Warasimbu+Travellers+Inn+Kundiawa+Papua+New+Guinea" target="_blank" rel="noopener" aria-label="Open the BNB location in Google Maps"><span><i class="fas fa-map-marked-alt"></i><br>View on Google Maps<br><small>Warasimbu Travellers Inn, Kundiawa</small></span></a><p><a class="btn btn-outline" href="https://www.google.com/maps/search/?api=1&query=Warasimbu+Travellers+Inn+Kundiawa+Papua+New+Guinea" target="_blank" rel="noopener">Get Directions</a></p>';document.querySelector('.contact-form')?.remove();
  }
